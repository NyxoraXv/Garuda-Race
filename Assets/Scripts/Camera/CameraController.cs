...
namespace CameraGame
{
    ...
    public class CameraController : MonoBehaviour
    {
        ...
        private Vector3 freeViewRotation = Vector3.zero;

        // Add this method to toggle the free view camera
        public void ToggleFreeView()
        {
            ...
            if (isFreeView)
            {
                // Capture the current rotation when entering free view
                freeViewRotation = transform.eulerAngles;
            }
        }

        // Add this method to control the free view camera
        private void HandleFreeView()
        {
            float mouseX = Input.GetAxis("Mouse X");
            float mouseY = Input.GetAxis("Mouse Y");

            // Adjust the free view rotation based on mouse input
            freeViewRotation.x -= mouseY;
            freeViewRotation.y += mouseX;

            // Limit the vertical rotation to prevent camera flipping
            freeViewRotation.x = Mathf.Clamp(freeViewRotation.x, -90f, 90f);

            // Apply the rotation to the camera
            transform.rotation = Quaternion.Euler(freeViewRotation);
        }
        ...
        */
        ...
        private void Update()
        {
            ...
            // Check for free view camera toggle
